...
<<echo = F, fig = T>>=
grid <- 1:100/10
chem_haz <- 5 / (1+grid) / 20
untrt_haz <- (1 + grid ^ 1.2 * .25)/ 20
y_lims <- range( c(chem_haz, untrt_haz)  )
plot(grid, chem_haz, type = 'l', lwd = 2, col = 'blue',
     ylim = y_lims, xlab = 'Time from Treatment', ylab = 'Hazard Rate',
     main = 'Hazard Rates for Chemo vs Untreated')
lines(grid, untrt_haz, lwd = 2, col = 'red')
legend('top', 
       c('Chemotherapy', 'Untreated'), 
       lwd = 2,
       col = c('blue', 'red'))
@
...
\begin{center}
<<echo = F, fig = T>>=    
hzd2surv <- function(hzd, time){
  trp_hzd <- (hzd[-1] + hzd[-length(hzd)])/2
  dt <- (time[-1] - time[-length(time)])
  cum_hzd <- c(0, cumsum(trp_hzd * dt) )
  return(exp(-cum_hzd))
}

chem_surv <- hzd2surv(chem_haz, grid)
untrt_surv <- hzd2surv(untrt_haz, grid)

plot(grid, chem_surv, ylim = c(0,1),
     xlab = 'Time from Treatment',
     ylab = 'S(t)',
     lwd = 2, 
     type = 'l', 
     col = 'blue',
     main = 'Survival Curves for Chemo vs Untreated')
lines(grid, untrt_surv, 
      lwd = 2, col = 'red')
legend('top', 
       c('Chemotherapy', 'Untreated'), 
       lwd = 2,
...
<<echo = F, fig = T>>=
grid <- 1:100
nonSmokeHaz <- (grid/40) ^ 2 / 400
smokeHaz    <- nonSmokeHaz * 2

plot(grid, smokeHaz, type = 'l', lwd = 2, col = 'red',
     xlab = 'Age', ylab = 'Hazard Rate',
     main = 'Hazard Rates for Lung Cancer\nSmokers vs Non-smokers')
lines(grid, nonSmokeHaz, lwd = 2, col = 'blue')
legend('top', 
       c('Smoker', 'Non-Smoker'), 
       lwd = 2,
       col = c('red', 'blue'))
...
\begin{center}
<<echo = F, fig = T>>=
plot(grid, hzd2surv(smokeHaz, grid), type = 'l', lwd = 2, col = 'red',
     xlab = 'Age', ylab = 'S(t)',
     main = 'Survival Curves for Lung Cancer\nSmokers vs Non-smokers',
     ylim = c(0,1))
lines(grid, hzd2surv(nonSmokeHaz, grid), lwd = 2, col = 'blue')
legend('topright', 
       c('Smoker', 'Non-Smoker'), 
       lwd = 2,
       col = c('red', 'blue'))
...
<<echo = F, fig = T>>=
grid <- 1:100/75
size_1 <- 1 - pweibull(grid, shape = 4)
size_2 <- 1 - pweibull(grid * exp(log(2) * 1), shape = 4)
size_4 <- 1 - pweibull(grid * exp(log(4) * 1), shape = 4)
  
  
plot(grid, size_1, type = 'l', lwd = 2, col = 'red',
     xlab = 'Time', ylab = 'S(t)',
     main = 'Probability of Completion by Crew Size',
     ylim = c(0,1))
lines(grid, size_2, lwd = 2, col = 'blue')
lines(grid, size_4, lwd = 2, col = 'darkgreen')
legend('topright', 
       paste('Crew size =', c(1, 2, 4)), 
       lwd = 2,
