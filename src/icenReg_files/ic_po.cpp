...
double icph_llk_sumOnly(ICPO_OptimInfo &optinfo){
...
double icph_llk_keepEta(ICPO_OptimInfo &optinfo){
...
void update_p_ob(int i, ICPO_OptimInfo &optinfo){
    int l,r, k;
    l = optinfo.lind[i];
    r = optinfo.rind[i];
    ...
    double thisS = optinfo.S[l];
    double this_p = (thisS * thisEE) / (thisS * thisEE - thisS + 1);
    
    thisS = optinfo.S[r+1];
    if( (r+1) < k )
        this_p -= (thisS * thisEE) / (thisS * thisEE - thisS + 1);
    optinfo.P_obs[i] = this_p;
}

void easyExchange(int p1, int p2, double delta, ICPO_OptimInfo &optinfo){
    optinfo.p_mass[p1] += delta;
    optinfo.p_mass[p2] -= delta;
}


void NNE_numeric_der(int p1, int p2, vector<double> &dvec, ICPO_OptimInfo &optinfo){
    double h = optinfo.h;
    double p1_s, p2_s;
...
void NNE_optim(int p1, int p2, ICPO_OptimInfo &optinfo){
...
void NNE_all(ICPO_OptimInfo &optinfo){
...
void vem_update(ICPO_OptimInfo &optinfo, bool recPosd){
...
void vec_delta(vector<double> &delta, ICPO_OptimInfo &optinfo){
    //    int n = optinfo.P_obs.size();
    int k = delta.size();
    //    vector<double>* S;
...
void analyticDerv_ICM(vector<double> &d1, vector<double> &d2, ICPO_OptimInfo  &optinfo){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(!r0)             rightSide = b / pob;
        else                rightSide = 0;
        l_d_cont[i] = leftSide;
        r_d_cont[i] = rightSide;
        l_d2_cont[i] = (1 - b) * b * R_pow(pob, -2);
        if(!r0)             r_d2_cont[i] = -(1 - b) * b * R_pow(pob, -2);
        else                r_d2_cont[i] = 0;
    }
    int ind;
    vector<bool>* gl;
...
void ICM_step(ICPO_OptimInfo &optinfo){
    int k = optinfo.p_mass.size();
    vector<double> d1(k);
    vector<double> d2(k);
...
void update_etas(ICPO_OptimInfo &optinfo){
...
void covar_d_analytic(Eigen::VectorXd &d1, Eigen::MatrixXd &dmat, ICPO_OptimInfo &optinfo){
    ...
    double left, right, left2, right2, s, expEta, pob;
    int l, r;
    for(int i = 0; i < n; i++){
        l = (*lind)[i];
        r = (*rind)[i];
...
double uni_cov_step(int i, double d1, ICPO_OptimInfo &optinfo){
...
SEXP ic_po(SEXP Rp_mass, SEXP Rlind, SEXP Rrind, SEXP Rcovars){
    ICPO_OptimInfo optObj (Rp_mass, Rlind, Rrind, Rcovars);
    update_etas(optObj);
...
ICPO_OptimInfo::ICPO_OptimInfo(SEXP Rp_mass, SEXP Rlind, SEXP Rrind, SEXP Rcovars){
    int n = LENGTH(Rlind);
    if(n != LENGTH(Rrind)){
